...
APPLICATION_BASELINE_SOURCECODE_REPOSITORY="`${BUILD_HOME}/helpers/GetVariableValue.sh APPLICATION_BASELINE_SOURCECODE_REPOSITORY`"
...
then
        ws_ips="`${BUILD_HOME}/services/server/GetServerPrivateIPAddresses.sh "ws-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`"
        db_ip="`${BUILD_HOME}/services/server/GetServerPrivateIPAddresses.sh "db-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`"
else
        ws_ips="`${BUILD_HOME}/services/server/GetServerIPAddresses.sh "ws-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`"
        db_ip="`${BUILD_HOME}/services/server/GetServerIPAddresses.sh "db-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`"
fi
...
then
        if ( [ "`/bin/grep "^BUILDCHAINTYPE:*" ${BUILD_HOME}/configuration/software.dat | /usr/bin/awk -F':' '{print $NF}'`" = "standard" ] || [ "`/bin/grep "^BUILDCHAINTYPE:*" ${BUILD_HOME}/configuration/software.dat | /usr/bin/awk -F':' '{print $NF}'`" = "database" ] )
        then
              #  if ( [ "${HARDCORE}" = "1" ] )
              #  then
              #          if ( [ "${BUILD_ARCHIVE_CHOICE}" = "virgin" ] )
              #          then
              #                  /bin/echo "Database name: `${BUILD_HOME}/helpers/GetVariableValue.sh 'DB_NAME'`" > ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/credentials/application_credentials.dat
              #                  /bin/echo "Database username: `${BUILD_HOME}/helpers/GetVariableValue.sh 'DB_USERNAME'`" >> ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/credentials/application_credentials.dat
              #                  /bin/echo "Database password: `${BUILD_HOME}/helpers/GetVariableValue.sh 'DB_PASSWORD'`" >> ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/credentials/application_credentials.dat
              #                  /bin/echo "The database public IP address is: `${BUILD_HOME}/services/server/GetServerIPAddresses.sh "db-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`" >> ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/credentials/application_credentials.dat
              #                  /bin/echo "The database private IP address is: `${BUILD_HOME}/services/server/GetServerPrivateIPAddresses.sh "db-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"` (try this one first from your application if it timesout, try the public one)" >> ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/credentials/application_credentials.dat
              #                  /bin/echo "The database port is ${DB_PORT}" >> ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/credentials/application_credentials.dat
              #                  /bin/echo "You can make up your own database prefix but make sure to include the '_' character at the end of your prefix (for example 'dbprefix_')" >> ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/credentials/application_credentials.dat
              ...
              #          if ( [ "${BUILD_ARCHIVE_CHOICE}" = "virgin" ] )
              #          then
              #                  /bin/echo "Database name: `${BUILD_HOME}/helpers/GetVariableValue.sh 'DB_NAME'`" >&3
              #                  /bin/echo "Database username: `${BUILD_HOME}/helpers/GetVariableValue.sh 'DB_USERNAME'`" >&3
              #                  /bin/echo "Database password: `${BUILD_HOME}/helpers/GetVariableValue.sh 'DB_PASSWORD'`" >&3
              #          fi
              #  fi
                ...
                status "########################################################################################################################"
                status "The database public IP address is: `${BUILD_HOME}/services/server/GetServerIPAddresses.sh "db-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`"
                status "The database private IP address is: `${BUILD_HOME}/services/server/GetServerPrivateIPAddresses.sh "db-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"` (try this one first from your application if it timesout, try the public one)"
                ...
                if ( [ "${APPLICATION}" = "joomla" ] && [ "`/bin/grep "INTERACTIVE_APPLICATION_INSTALL:yes" ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/application/${APPLICATION}.dat | /usr/bin/awk -F':' '{print $NF}'`" != "" ]  )
                then
                        status ""
                        status "##################################################################################################"
