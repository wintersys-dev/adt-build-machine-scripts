...
# Templates are held in the "${BUILD_HOME}/initialisation/configfiles" subdirectory
##################################################################################
# License Agreement:
...
TOKEN="`${BUILD_HOME}/helpers/GetVariableValue.sh TOKEN`"
CLOUDHOST_ACCOUNT_ID="`${BUILD_HOME}/helpers/GetVariableValue.sh CLOUDHOST_ACCOUNT_ID`"
REGION="`${BUILD_HOME}/helpers/GetVariableValue.sh REGION`"
ACCESS_KEY="`${BUILD_HOME}/helpers/GetVariableValue.sh ACCESS_KEY`"
SECRET_KEY="`${BUILD_HOME}/helpers/GetVariableValue.sh SECRET_KEY`"
DNS_SECURITY_KEY="`${BUILD_HOME}/helpers/GetVariableValue.sh DNS_SECURITY_KEY`"
...
then
	...
	/bin/cp ${BUILD_HOME}/initialisation/configfiles/digitalocean/digitalocean.tmpl ${BUILD_HOME}/.config/doctl/config.yaml
	...
	/bin/echo "${TOKEN}" > ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/TOKEN
	/bin/chmod 700 ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/TOKEN
	...
	if ( [ "`${BUILD_HOME}/helpers/IsHardcoreBuild.sh`" = "0" ] )
	then
...
then
	...
	/bin/cp ${BUILD_HOME}/initialisation/configfiles/exoscale/exoscale.tmpl  ${BUILD_HOME}/.exoscale.toml
	...
	then 
		...
		#	/bin/echo "${ACCESS_KEY}" > ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/ACCESS_KEY
	else 
	...
	then
		...
		#	/bin/echo "${SECRET_KEY}" > ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/SECRET_KEY
	else 
...
then
	...
	/bin/cp ${BUILD_HOME}/initialisation/configfiles/linode/linode-cli.tmpl  ${BUILD_HOME}/.linode-cli
...
then
	...
	then
		...
		/bin/chown root:root ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/TOKEN ${BUILD_HOME}/.vultr-cli.yaml /root/.vultr-cli.yaml
		/bin/chmod 400 ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/TOKEN ${BUILD_HOME}/.vultr-cli.yaml /root/.vultr-cli.yaml

		/bin/echo ${DNS_SECURITY_KEY} > ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/DNS_TOKEN
		/bin/echo "api-key: ${DNS_SECURITY_KEY}" > ${BUILD_HOME}/.dns-vultr-cli.yaml
		/bin/echo "api-key: ${DNS_SECURITY_KEY}" > /root/.dns-vultr-cli.yaml
		/bin/chown root:root ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/DNS_TOKEN ${BUILD_HOME}/.dns-vultr-cli.yaml /root/.dns-vultr-cli.yaml
		/bin/chmod 400 ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/DNS_TOKEN ${BUILD_HOME}/.dns-vultr-cli.yaml /root/.dns-vultr-cli.yaml
	else
...
/bin/echo "${CLOUDHOST}" > ${BUILD_HOME}/runtime/BUILD_MACHINE_CLOUDHOST
