...
# Templates are held in the "${BUILD_HOME}/initialisation/configfiles/datastore" subdirectory
##################################################################################
# License Agreement:
...
#${BUILD_HOME}/installation/InstallDatastoreTools.sh "${BUILDOS}" 2>&1 >/dev/null
...
then
        ...
        /bin/cp ${BUILD_HOME}/initialisation/configfiles/datastore/s3-cfg.tmpl  ${BUILD_HOME}/.s3cfg-${count}
...
if ( [ "`/bin/grep "^DATASTORETOOL:*" ${BUILD_HOME}/configuration/software.dat | /bin/grep rclone`" != "" ] )
then
        ...
        /bin/cp ${BUILD_HOME}/initialisation/configfiles/datastore/rclone-cfg.tmpl ${BUILD_HOME}/.rclone.cfg-${count}
...
${BUILD_HOME}/services/datastore/operations/MountDatastore.sh "ad-hoc" 3>&1 2>/dev/null
${BUILD_HOME}/services/datastore/operations/DeleteDatastore.sh "ad-hoc" 3>&1 2>/dev/null
...
then
        ...
        if ( [ "`${BUILD_HOME}/services/datastore/operations/ListFromDatastore.sh "multi-region"`" != "" ] && [ "`${BUILD_HOME}/helpers/services/IsHardcoreBuild.sh`" != "1" ] )
        then
                status "####################HALT################################"
...
then
        for bucket in `${BUILD_HOME}/services/datastore/operations/ListDatastore.sh "config-all"`
        do
...
then
        for bucket in `${BUILD_HOME}/services/datastore/operations/ListDatastore.sh "auth-config"`
        do
                additional_specifier="`/bin/echo ${bucket} | /usr/bin/awk -F'-' '{print $NF}'`"
                ${BUILD_HOME}/services/datastore/operations/DeleteDatastore.sh "auth-config" "local" "${additional_specifier}"
        done
fi
