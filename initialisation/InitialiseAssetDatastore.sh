...
DIRECTORIES_TO_MOUNT="`${BUILD_HOME}/helpers/GetVariableValue.sh DIRECTORIES_TO_MOUNT | /bin/sed 's/:/ /g'`"
...
then
        ...
        do
                ...
                then
                        ...
                        ${BUILD_HOME}/services/datastore/operations/MountDatastore.sh "${asset_datastore}"

                        if ( [ -f ${interrogation_home}/${subdir} ] )
                        then
                                if ( [ ! -z "`/bin/ls ${interrogation_home}/${subdir}`" ] )
                                then
                                        ${BUILD_HOME}/services/datastore/operations/SyncDatastore.sh ${interrogation_home}/${subdir}/ ${asset_datastore}
                                fi
                        fi
                fi
        done
