...
then
	...
	then
		/bin/rm ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/STATIC_SCALE:*
	fi
...
#	new_scale_values="`/bin/echo "${new_scale_values}" | /bin/sed 's/:/ /g'`"
	...
	${BUILD_HOME}/services/datastore/operations/MountDatastore.sh "scaling" "local" "scaling-${CLOUDHOST}-${REGION}"
...
#	do
#		if ( [ ! -d ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/scaling/autoscaler-${no_autoscaler} ] )
#		then
#			/bin/mkdir -p ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/scaling/autoscaler-${no_autoscaler}
#		fi
#		scale_value="`/bin/echo ${new_scale_values} | /usr/bin/cut -d' ' -f${no_autoscaler}`"
#		/bin/touch ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/scaling/autoscaler-${no_autoscaler}/STATIC_SCALE:${scale_value}
#		${BUILD_HOME}/services/datastore/operations/DeleteFromDatastore.sh "scaling" "*autoscaler-${no_autoscaler}*" "local" "scaling-${CLOUDHOST}-${REGION}"
#		${BUILD_HOME}/services/datastore/operations/PutToDatastore.sh "scaling" "${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/scaling/autoscaler-${no_autoscaler}/STATIC_SCALE:${scale_value}" "autoscaler-${no_autoscaler}" "local" "no" "scaling-${CLOUDHOST}-${REGION}"
