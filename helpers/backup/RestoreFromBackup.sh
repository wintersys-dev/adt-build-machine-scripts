...
${BUILD_HOME}/helpers/servers/ExecuteOnWebserver.sh "${SUDO} ${command}"
command='/bin/grep -rlZ "'${DB_NAME}'" '/var/www/html' | /usr/bin/xargs -0 /bin/sed -i "s/'${DB_NAME}'.*/'${DB_NAME}'_restored-${restoration_no}/g"'
${BUILD_HOME}/helpers/servers/ExecuteOnWebserver.sh "${SUDO} ${command}"
${BUILD_HOME}/helpers/servers/ExecuteOnWebserver.sh "${SUDO} ${HOME}/application/InstallApplication.sh"
${BUILD_HOME}/helpers/servers/ExecuteOnWebserver.sh "${SUDO} ${HOME}/application/configuration/InitialiseApplicationConfiguration.sh"

command='/bin/grep -rlZ "'${DB_NAME}'" '${HOME}' | /usr/bin/xargs -0 /bin/sed -i "s/'${DB_NAME}'.*/'${DB_NAME}'_restored-${restoration_no}/g"'
${BUILD_HOME}/helpers/servers/ExecuteOnDatabase.sh "${SUDO} ${command}"
${BUILD_HOME}/helpers/servers/ExecuteOnDatabase.sh "${SUDO} ${HOME}/application/db/InstallApplicationDB.sh"
