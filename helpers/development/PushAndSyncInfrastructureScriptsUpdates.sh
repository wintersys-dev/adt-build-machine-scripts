...
if ( [ "${branch}" = "" ] )
then
        BRANCH="`/bin/grep "^GITBRANCH:" ${BUILD_HOME}/configuration/software.dat | /usr/bin/awk -F':' '{print $NF}'`"
else
        BRANCH="${branch}"
fi
...
GIT_USER="`/bin/sh ${BUILD_HOME}/helpers/services/GetVariableValue.sh GIT_USER`"
GIT_EMAIL_ADDRESS="`/bin/sh ${BUILD_HOME}/helpers/services/GetVariableValue.sh GIT_EMAIL_ADDRESS`"
...
/bin/sh ${BUILD_HOME}/installation/InstallRsync.sh "`/bin/cat /etc/issue | /usr/bin/tr '[:upper:]' '[:lower:]' | /bin/egrep -o '(ubuntu|debian)'`"
