...
# Author: Peter Winter
# Date  : 13/07/2016
# Description : This script will copy a specified file from a remote machine to a local directory 
##########################################################################################################
# License Agreement:
...
if ( [ "${sourcefile}" = "" ] )
then
        /bin/echo "Please tell me the full path to the location of the file you wish to copy from the remote machine for example, \${HOME}/migrationdirectory/archive.tar.gz"
        read sourcefile
        while ( [ "`/bin/ls ${sourcefile}`" = "" ] )
        do
                /bin/echo "Sorry, can't find that file please tell me again"
                /bin/echo "Please tell me the full path to the location of the file you wish from the remote machine for example, \${HOME}/migrationdirectory/archive.tar.gz"
                read sourcefile
        done
fi 

/bin/echo "Please enter the full path to the directory you would like to copy the file to on the build machine, for example ${BUILD_HOME}"
read localdir
...
/usr/bin/scp -o ConnectTimeout=5 -o ConnectionAttempts=2 -o UserKnownHostsFile=${MACHINE_PUBLIC_KEYS} -o StrictHostKeyChecking=yes -P ${SSH_PORT} -i ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/keys/id_${ALGORITHM}_AGILE_DEPLOYMENT_BUILD_KEY_${BUILD_IDENTIFIER} ${SERVER_USER}@${MACHINE_IP}:${sourcefile} ${localdir}
