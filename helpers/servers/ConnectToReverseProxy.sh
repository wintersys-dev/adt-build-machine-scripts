...
# Author: Peter Winter
# Date  : 13/07/2016
# Description : This script will connect you to your reverse proxy machine through its configured port via ssh
######################################################################################################################################################
# License Agreement:
...
token_to_match="rp-`${BUILD_HOME}/helpers/services/GetVariableValue.sh REGION`-${BUILD_IDENTIFIER}"
...
if ( [ "${ips}" = "" ] )
then
	/bin/echo "There doesn't seem to be any reverseproxies running"
	exit
fi
