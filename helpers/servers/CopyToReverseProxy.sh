...
# Author : Peter Winter
# Date   : 13/07/2016
# Description : This script will copy a file, passed as a parameter to your selected reverseproxy
######################################################################################################
# License Agreement:
...
if ( [ "${sourcefile}" = "" ] )
then
        /bin/echo "Please tell me the full path to the location of the file you wish to copy to the reverseproxy for example, ${BUILD_HOME}/migrationdirectory/archive.tar.gz"
        read sourcefile
...
then
	...
	then
		...
		/bin/echo "OK, which reverseproxy would you like to copy your file to?"
		count="1"
		for ip in ${ips}
		do
			if ( [ "${ip_selected}" = "0" ] )
...
start=`/bin/date +%s`
/usr/bin/scp -o ConnectTimeout=5 -o ConnectionAttempts=2 -o UserKnownHostsFile=${REVERSEPROXY_PUBLIC_KEYS} -o StrictHostKeyChecking=yes -P ${SSH_PORT} -i ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/keys/id_${ALGORITHM}_AGILE_DEPLOYMENT_BUILD_KEY_${BUILD_IDENTIFIER} ${sourcefile} ${SERVER_USER}@${REVERSEPROXY_IP}:${remotedir}
end=`/bin/date +%s`
