...
# Author : Peter Winter
# Date   : 13/07/2016
# Description : This script will copy a file, passed as a parameter to your selected reverseproxy server
####################################################################################################
# License Agreement:
...
if ( [ "${sourcefile}" = "" ] )
then
        /bin/echo "Please tell me the full path to the location of the file you wish to copy from the reverseproxy server, for example, /tmp/reverseproxydump.tar.gz"
        read sourcefile
fi
...
start=`/bin/date +%s`
/usr/bin/scp -o ConnectTimeout=5 -o ConnectionAttempts=2 -o UserKnownHostsFile=${REVERSEPROXY_PUBLIC_KEYS} -o StrictHostKeyChecking=yes -P ${SSH_PORT} -i ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/keys/id_${ALGORITHM}_AGILE_DEPLOYMENT_BUILD_KEY_${BUILD_IDENTIFIER} ${SERVER_USER}@${REVERSEPROXY_IP}:${sourcefile} ${localdir}
end=`/bin/date +%s`
