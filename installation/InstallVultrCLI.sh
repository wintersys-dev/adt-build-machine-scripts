...
then
    if ( [ "${buildos}" = "ubuntu" ] )
    then 
        if ( [ "`/bin/grep "^CLOUDCLITOOL:*" ${BUILD_HOME}/configuration/software.dat | /bin/grep CLOUDCLITOOL:vultr:binary`" != "" ] )
        then
        ...
        elif ( [ "`/bin/grep "^CLOUDCLITOOL:*" ${BUILD_HOME}/configuration/software.dat | /bin/grep CLOUDCLITOOL:vultr:source`" != "" ] )
        then
