...
then
	...
	${BUILD_HOME}/installation/InstallDoctl.sh "${buildos}"
fi

if ( [ "${cloudhost}" = "exoscale" ] )
then
	status "Installing/Updating Exoscale cloudhost cli tool (exo)"
	${BUILD_HOME}/installation/InstallExo.sh "${buildos}"
fi

if ( [ "${cloudhost}" = "linode" ] )
then
	status "Installing/Updating Linode cloudhost cli tool (linode-cli)"
	${BUILD_HOME}/installation/InstallLinodeCLI.sh "${buildos}"
fi

if ( [ "${cloudhost}" = "vultr" ] )
then
	status "Installing/Updating Vultr cloudhost cli tool (vultr-cli)"
	${BUILD_HOME}/installation/InstallSudo.sh "${buildos}"
	${BUILD_HOME}/installation/InstallVultrCLI.sh "${buildos}"        
fi
