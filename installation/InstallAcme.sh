...
then
	...
	if ( [ "${buildos}" = "ubuntu" ] )
	then
		if ( [ "`/bin/grep "^SSLCERTCLIENT:*" ${BUILD_HOME}/configuration/software.dat | /bin/grep SSLCERTCLIENT:acme:acme.sh`" != "" ] )
		then
			/usr/bin/wget -O -  https://get.acme.sh | /bin/sh -s email=${email} 
		elif ( [ "`/bin/grep "^SSLCERTCLIENT:*" ${BUILD_HOME}/configuration/software.dat | /bin/grep SSLCERTCLIENT:acme:github.com`" != "" ] )
		then
