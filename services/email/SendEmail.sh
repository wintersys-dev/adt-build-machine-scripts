...
then
        ip_address="`${BUILD_HOME}/helpers/GetBuildMachineIP.sh`"
        message="MESSAGE RELATED TO MACHINE WITH IP ADDRESS: ${ip_address}: ${message}"
fi

FROM_ADDRESS="`${BUILD_HOME}/helpers/GetVariableValue.sh SYSTEM_FROMEMAIL_ADDRESS`"
FROM_NAME="`${BUILD_HOME}/helpers/GetVariableValue.sh WEBSITE_DISPLAYNAME | /usr/bin/sed 's/_//g'`"
TO_ADDRESS="`${BUILD_HOME}/helpers/GetVariableValue.sh SYSTEM_TOEMAIL_ADDRESS`"
...
USERNAME="`${BUILD_HOME}/helpers/GetVariableValue.sh SYSTEM_EMAIL_USERNAME`"
PASSWORD="`${BUILD_HOME}/helpers/GetVariableValue.sh SYSTEM_EMAIL_PASSWORD`"
EMAIL_PROVIDER="`${BUILD_HOME}/helpers/GetVariableValue.sh SYSTEM_EMAIL_PROVIDER`"

if ( [ "${level}" != "MANDATORY" ] && [ "`${BUILD_HOME}/helpers/GetVariableValue.sh EMAIL_NOTIFICATION_LEVEL`" = "ERROR" ] && [ "${level}" != "ERROR" ] )
then
        exit
fi
...
then
        if ( [ "`/bin/grep "^EMAILUTIL:*" ${BUILD_HOME}/configuration/software.dat | /bin/grep EMAILUTIL:sendemail`" != "" ] )
        then
                if ( [ "${EMAIL_PROVIDER}" = "1" ] )
                then
        ...
        if ( [ "`/bin/grep "^EMAILUTIL:*" ${BUILD_HOME}/configuration/software.dat | /bin/grep EMAILUTIL:ssmtp`" != "" ] )
        then
