...
SSL_LIVE_CERT="`${BUILD_HOME}/helpers/services/GetVariableValue.sh SSL_LIVE_CERT`"
...
if ( [ ! -f /usr/bin/lego ] )
then
        ${BUILD_HOME}/installation/InstallLego.sh ${BUILDOS}
fi
...
then
        ...
        then
                ...
                command='CLOUDFLARE_DNS_API_TOKEN="'${cloudflare_dns_api_token}'" /usr/bin/lego lego run --email '${DNS_USERNAME}' --domains '${WEBSITE_URL}' --dns '${DNS_CHOICE}' '${server}' --dns.resolvers '1.1.1.1:53,8.8.8.8:53' --accept-tos'
        else
                command='CF_API_EMAIL="'${DNS_USERNAME}'" CF_API_KEY="'${DNS_SECURITY_KEY}'" /usr/bin/lego run --email '${DNS_USERNAME}' --domains '${WEBSITE_URL}' --dns '${DNS_CHOICE}' '${server}' --dns.resolvers '1.1.1.1:53,8.8.8.8:53' --accept-tos'
        fi
fi

if ( [ "${DNS_CHOICE}" = "digitalocean" ] )
then
        command="DO_AUTH_TOKEN="${DNS_SECURITY_KEY}"  DO_POLLING_INTERVAL=30 DO_PROPAGATION_TIMEOUT=600 /usr/bin/lego run --email ${DNS_USERNAME} --domains ${WEBSITE_URL} --dns ${DNS_CHOICE} ${server} --dns.resolvers '1.1.1.1:53,8.8.8.8:53' --accept-tos"
fi
...
then
        ...
        command="EXOSCALE_API_KEY=${EXOSCALE_API_KEY} EXOSCALE_API_SECRET=${EXOSCALE_API_SECRET} EXOSCALE_POLLING_INTERVAL=30 EXOSCALE_PROPAGATION_TIMEOUT=600 /usr/bin/lego run --email ${DNS_USERNAME} --dns ${DNS_CHOICE} ${server} --domains ${WEBSITE_URL} --dns.resolvers '1.1.1.1:53,8.8.8.8:53' --accept-tos"
fi

if ( [ "${DNS_CHOICE}" = "linode" ] )
then
        #LINODE_TOKEN="`/bin/echo ${DNS_SECURITY_KEY} | /usr/bin/awk -F':' '{print $1}'`"
        command="LINODE_TOKEN=${DNS_SECURITY_KEY}  LINODE_POLLING_INTERVAL=30 LINODE_PROPAGATION_TIMEOUT=600 LINODE_HTTP_TIMEOUT=120 /usr/bin/lego run --email ${DNS_USERNAME} --dns ${DNS_CHOICE} ${server} --domains ${WEBSITE_URL} --cert.timeout 120    --dns.resolvers '1.1.1.1:53,8.8.8.8:53' --accept-tos"
fi

if ( [ "${DNS_CHOICE}" = "vultr" ] )
then
        #VULTR_API_KEY="`/bin/echo ${DNS_SECURITY_KEY} | /usr/bin/awk -F':' '{print $1}'`"
        command="VULTR_API_KEY=${DNS_SECURITY_KEY} VULTR_POLLING_INTERVAL=30 VULTR_PROPAGATION_TIMEOUT=600  LEGO_DISABLE_CNAME_SUPPORT=true /usr/bin/lego run --email ${DNS_USERNAME} --dns ${DNS_CHOICE} ${server} --domains ${WEBSITE_URL} --dns.resolvers '1.1.1.1:53,8.8.8.8:53' --accept-tos"
fi
...
then
        ...
        do
                status "Attempting to generate SSL Certificate, this is attempt ${count} of 5"
                eval ${command}
                ...
                then
                        ...
                        then
                                ...
                                /bin/cp /var/snap/lego/common/.lego/certificates/${WEBSITE_URL}.crt ${BUILD_HOME}/.lego/certificates/${WEBSITE_URL}.crt
                                /bin/cp /var/snap/lego/common/.lego/certificates/${WEBSITE_URL}.key ${BUILD_HOME}/.lego/certificates/${WEBSITE_URL}.key
                        fi
                fi
