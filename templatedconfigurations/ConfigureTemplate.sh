...
if ( [ "`${BUILD_HOME}/helpers/services/IsHardcoreBuild.sh`" != "1" ] && [ ! -f /root/PARAMETER ] )
then
	status ""
...
elif ( [ "`${BUILD_HOME}/helpers/services/IsHardcoreBuild.sh`" = "1" ] && [ ! -f /root/PARAMETER ] )
then
	. ${BUILD_HOME}/runtime/${cloudhost}/${build_identifier}/build_environment
	templatefile="${BUILD_HOME}/templatedconfigurations/templates/${cloudhost}/${cloudhost}${selectedtemplate}.tmpl"

	if ( [ ! -d ${BUILD_HOME}/runtime/${cloudhost}/${build_identifier}/hardcoretemplates ] )
	then
		/bin/mkdir -p  ${BUILD_HOME}/runtime/${cloudhost}/${build_identifier}/hardcoretemplates
	fi

	if ( [ -f ${BUILD_HOME}/runtime/${cloudhost}/${build_identifier}/hardcoretemplates/${cloudhost}${selectedtemplate}.tmpl ] )
	then
		/bin/mv ${BUILD_HOME}/runtime/${cloudhost}/${build_identifier}/hardcoretemplates/${cloudhost}${selectedtemplate}.tmpl ${BUILD_HOME}/runtime/${cloudhost}/${build_identifier}/hardcoretemplates/${cloudhost}${selectedtemplate}.tmpl.$$
	fi

	/bin/cp ${templatefile} ${BUILD_HOME}/runtime/${cloudhost}/${build_identifier}/hardcoretemplates/${cloudhost}${selectedtemplate}.tmpl
	templatefile="${BUILD_HOME}/runtime/${cloudhost}/${build_identifier}/hardcoretemplates/${cloudhost}${selectedtemplate}.tmpl"
...
#Make it live
if ( [ ! -d ${BUILD_HOME}/runtime/${cloudhost}/${build_identifier} ] )
then
	/bin/mkdir -p ${BUILD_HOME}/runtime/${cloudhost}/${build_identifier}
fi
/bin/cp ${templatefile} ${BUILD_HOME}/runtime/${cloudhost}/${build_identifier}
/bin/echo ${templatefile} > ${BUILD_HOME}/runtime/current_template_name
