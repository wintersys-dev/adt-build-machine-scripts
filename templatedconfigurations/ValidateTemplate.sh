...
if ( [ "${MOD_SECURITY}" = "1" ] && [ "${WEBSERVER_CHOICE}" = "NGINX" ] && [ "`/bin/grep "^NGINX:source" ${BUILD_HOME}/configuration/software.dat`" = "" ] )
then
	${log_command} "To use NGINX with modsecurity enabled NGINX needs to be compiled from source which you can configure in ${BUILD_HOME}/configuration/software.dat"
fi
...
then
	${log_command} "You are in development mode, NO_AUTOSCALERS should be 0 not  (${NO_AUTOSCALERS}) and so doesn't appear to be valid please review"
  	${BUILD_HOME}/helpers/SetVariableValue.sh "NO_AUTOSCALERS=0"
fi
...
then
	${log_command} "In development mode, the number of webservers has to be 1"
  	${BUILD_HOME}/helpers/SetVariableValue.sh "NO_WEBSERVERS=1"
fi
...
if ( [ "`/bin/grep "^SSLCERTCLIENT:lego" ${BUILD_HOME}/configuration/software.dat`" != "" ] && [ "${SSL_GENERATION_SERVICE}" != "LETSENCRYPT" ] )
then
	${log_command} "There seems to be a mismatch between your SSL CLIENT software choice in ${BUILD_HOME}/configuration/software.dat"
...
if ( [ "`/bin/grep "^SSLCERTCLIENT:acme" ${BUILD_HOME}/configuration/software.dat`" != "" ] && [ "${SSL_GENERATION_SERVICE}" != "ZEROSSL" ] )
then
	${log_command} "There seems to be a mismatch between your SSL CLIENT software choice in ${BUILD_HOME}/configuration/software.dat"
 	${log_command} "and your chosen ssl generation service (${SSL_GENERATION_SERVICE})"
...
then
	if ( [ "`${BUILD_HOME}/services/git/GitLSRemote.sh ${APPLICATION_REPOSITORY_PROVIDER} ${APPLICATION_REPOSITORY_USERNAME} ${APPLICATION_REPOSITORY_OWNER} ${BASELINE_DB_REPOSITORY} ${APPLICATION_REPOSITORY_TOKEN}`" = "" ] )
	then
	...
	if ( [ "`${BUILD_HOME}/services/git/GitLSRemote.sh ${APPLICATION_REPOSITORY_PROVIDER} ${APPLICATION_REPOSITORY_USERNAME} ${APPLICATION_REPOSITORY_OWNER} ${APPLICATION_BASELINE_SOURCECODE_REPOSITORY} ${APPLICATION_REPOSITORY_TOKEND}`" = "" ] )
	then
...
if ( [ "${DATABASE_INSTALLATION_TYPE}" = "Postgres" ] && [ "`/bin/grep ^PHP ${BUILD_HOME}/configuration/software.dat | /bin/grep pgsql`" = "" ] )
then
	${log_command} "It looks like you are trying to install Postgres without PHP support for postgres (pgsql)"
fi

if ( [ "${DATABASE_INSTALLATION_TYPE}" = "Maria" ] && [ "`/bin/grep ^PHP ${BUILD_HOME}/configuration/software.dat | /bin/grep mysqli`" = "" ] )
then
	${log_command} "It looks like you are trying to install MariaDB without PHP support for mysql (mysqli)"
fi

if ( [ "${DATABASE_INSTALLATION_TYPE}" = "MySQL" ] && [ "`/bin/grep ^PHP ${BUILD_HOME}/configuration/software.dat | /bin/grep mysqli`" = "" ] )
then
	${log_command} "It looks like you are trying to install MySQL without PHP support for mysql (mysqli)"
fi

if ( [ "`/bin/echo ${DATABASE_DBaaS_INSTALLATION_TYPE} | /bin/grep MySQL`" != "" ] && [ "`/bin/grep ^PHP ${BUILD_HOME}/configuration/software.dat | /bin/grep mysqli`" = "" ] )
then
	${log_command} "It looks like you are trying to install MySQL without PHP support for mysql (mysqli)"
fi

if ( [ "`/bin/echo ${DATABASE_DBaaS_INSTALLATION_TYPE} | /bin/grep Postgres`" != "" ] && [ "`/bin/grep ^PHP ${BUILD_HOME}/configuration/software.dat | /bin/grep pgsql`" = "" ] )
then
	${log_command} "It looks like you are trying to install Postgres without PHP support for postgres (pgsql)"
fi
