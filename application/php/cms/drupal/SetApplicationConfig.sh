...
webroot_subdirectory="`/bin/grep "^WEBROOT_SUBDIRECTORY" ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/application/${APPLICATION}.dat | /usr/bin/awk -F':' '{print $NF}'`"

if ( [ "${websroot_subdirectory}" = "" ] )
then
	webroot_subdirectory="web"
fi

/bin/sed -i "s;XXXXWEBROOT_SUBDIRECTORYXXXX;${webroot_subdirectory};g" ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/application/${APPLICATION}.dat


XXXXWEBROOT_SUBDIRECTORYXXXX
