...
then
        ...
        do
                if ( [ "`${BUILD_HOME}/services/datastore/operations/ListFromDatastore.sh "asset" "root" "${assettype}" | /usr/bin/wc -l`" -gt "0" ] )
                then
                        ...
                        ${BUILD_HOME}/services/datastore/operations/DeleteDatastore.sh "asset" "local" "${assettype}"
                fi
        done
fi
