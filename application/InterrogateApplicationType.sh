...
WEBSITE_NAME="`${BUILD_HOME}/helpers/GetVariableValue.sh WEBSITE_NAME`"
BUILD_ARCHIVE_CHOICE="`${BUILD_HOME}/helpers/GetVariableValue.sh BUILD_ARCHIVE_CHOICE`"
APPLICATION_REPOSITORY_PROVIDER="`${BUILD_HOME}/helpers/GetVariableValue.sh APPLICATION_REPOSITORY_PROVIDER`"
APPLICATION_REPOSITORY_USERNAME="`${BUILD_HOME}/helpers/GetVariableValue.sh APPLICATION_REPOSITORY_USERNAME`"
APPLICATION_REPOSITORY_OWNER="`${BUILD_HOME}/helpers/GetVariableValue.sh APPLICATION_REPOSITORY_OWNER`"
APPLICATION_REPOSITORY_TOKEN="`${BUILD_HOME}/helpers/GetVariableValue.sh APPLICATION_REPOSITORY_TOKEN`"
...
then
        ...
        if ( [ "`${BUILD_HOME}/services/git/GitLSRemote.sh ${APPLICATION_REPOSITORY_PROVIDER} ${APPLICATION_REPOSITORY_USERNAME} ${APPLICATION_REPOSITORY_OWNER} ${APPLICATION_BASELINE_SOURCECODE_REPOSITORY} ${APPLICATION_REPOSITORY_TOKEN} 2>/dev/null`" = "" ] )
        then
        ...
        if ( [ "`${BUILD_HOME}/services/git/GitLSRemote.sh ${APPLICATION_REPOSITORY_PROVIDER} ${APPLICATION_REPOSITORY_USERNAME} ${APPLICATION_REPOSITORY_OWNER} ${BASELINE_DB_REPOSITORY} ${APPLICATION_REPOSITORY_TOKEN} 2>/dev/null`" = "" ] )
        then
                status "Sorry, could not find the baseline repository for you application when I was expecting to, will have to exit..."
                /bin/touch /tmp/END_IT_ALL
        else
...
then
        datastorebucket="0"
        datastoredbbucket="0"
        ${BUILD_HOME}/services/datastore/operations/GetFromDatastore.sh "backup-web" "applicationsourcecode.tar.gz" "." "${BUILD_ARCHIVE_CHOICE}"
        ${BUILD_HOME}/services/datastore/operations/GetFromDatastore.sh "backup-db" "${WEBSITE_NAME}-DB-backup.tar.gz" "." "${BUILD_ARCHIVE_CHOICE}"
...
if ( [ "${gitrepo}" = "1" ] )
then
        ${BUILD_HOME}/services/git/GitClone.sh ${APPLICATION_REPOSITORY_PROVIDER} ${APPLICATION_REPOSITORY_USERNAME} ${APPLICATION_REPOSITORY_OWNER} ${APPLICATION_BASELINE_SOURCECODE_REPOSITORY} ${APPLICATION_REPOSITORY_TOKEN}
        ${BUILD_HOME}/services/git/GitClone.sh ${APPLICATION_REPOSITORY_PROVIDER} ${APPLICATION_REPOSITORY_USERNAME} ${APPLICATION_REPOSITORY_OWNER} ${BASELINE_DB_REPOSITORY} ${APPLICATION_REPOSITORY_TOKEN}
