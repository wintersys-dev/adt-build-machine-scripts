...
PERSIST_ASSETS_TO_DATASTORE="`${BUILD_HOME}/helpers/GetVariableValue.sh PERSIST_ASSETS_TO_DATASTORE`"
...
DB_IDENTIFIER="`${BUILD_HOME}/helpers/GetVariableValue.sh DB_IDENTIFIER`"
...
if ( [ "`${BUILD_HOME}/services/server/ListServerIDs.sh "ws-${REGION}-${BUILD_IDENTIFIER}" ${CLOUDHOST}`" = "" ] )
then
...
if (  [ "${DATABASE_INSTALLATION_TYPE}" != "None" ] && [ "`${BUILD_HOME}/services/server/ListServerIDs.sh "db-${REGION}-${BUILD_IDENTIFIER}" ${CLOUDHOST}`" = "" ] )
then
...
then 
	...
	then
		...
		then
			/usr/bin/scp -q ${OPTIONS_AS} -i ${BUILD_KEY} -P ${SSH_PORT} ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/EMERGENCY_PASSWORD ${SERVER_USER}@${as_active_ip}:/home/${SERVER_USER}/.ssh/EMERGENCY_PASSWORD 2>/dev/null
		fi
	elif ( [ "${no_autoscalers}" != "0" ] )
	then
...
/bin/touch ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/INITIAL_BUILD_COMPLETED
...
then
	...
	then
		if ( [ "${rp_active_ips}" = "" ] )
		then
			rp_active_ips="`${BUILD_HOME}/services/server/GetServerPrivateIPAddresses.sh "rp-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`"
		fi
	fi
elif ( [ "${rp_active_ips}" = "" ] )
then
	rp_active_ips="`${BUILD_HOME}/services/server/GetServerIPAddresses.sh "rp-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`"
fi
...
if ( [ "${BUILD_MACHINE_VPC}" = "1" ] )
then
	ws_active_ips="`${BUILD_HOME}/services/server/GetServerPrivateIPAddresses.sh "ws-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`"
	rp_ws_active_ips="${ws_active_ips}"
else
	ws_active_ips="`${BUILD_HOME}/services/server/GetServerIPAddresses.sh "ws-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`"
	rp_ws_active_ips="`${BUILD_HOME}/services/server/GetServerPrivateIPAddresses.sh "ws-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`"
fi
...
then
	...
	if ( [ "${no_autoscalers}" = "1" ] )
	then
		/usr/bin/ssh -q -p ${SSH_PORT} -i ${BUILD_KEY} ${OPTIONS_WS} ${SERVER_USER}@${as_active_ip} "${SUDO} /home/${SERVER_USER}/services/dbaas/AdjustDBaaSFirewall.sh" 2>/dev/null
	elif ( [ "${no_autoscalers}" != "0" ] )
	then
        for as_active_ip in ${as_active_ips}
        do
...
then
	...
	do
		...
		do
			...
			application_language_installed="`/usr/bin/ssh -q -p ${SSH_PORT} -i ${BUILD_KEY} ${OPTIONS_WS} ${SERVER_USER}@${ws_active_ip} "/usr/bin/test -f /home/${SERVER_USER}/runtime/installedsoftware/InstallPHPBase.sh && /bin/echo 'APPLICATION_LANGUAGE'"`" >&3 
...
then
	...
	do
		...
		do
			/usr/bin/ssh -q -p ${SSH_PORT} -i ${BUILD_KEY} ${OPTIONS_WS} ${SERVER_USER}@${ws_active_ip} "${SUDO} /home/${SERVER_USER}/services/webserver/RestartWebserver.sh" 2>/dev/null
			webserver_running="`/usr/bin/ssh -q -p ${SSH_PORT} -i ${BUILD_KEY} ${OPTIONS_WS} ${SERVER_USER}@${ws_active_ip} "${SUDO} /home/${SERVER_USER}/services/webserver/IsAWebserverRunning.sh"`"
...
${BUILD_HOME}/services/datastore/operations/PutToDatastore.sh "config" "INSTALLED_SUCCESSFULLY" "root" "local" "no"
